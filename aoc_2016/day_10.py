...
while (not(finished)):
    ...
    for instruction in instructions:
        if instruction[:3] == "bot":
            n1 = instruction.index(" gives low to ")
            n2 = instruction.index(" and high to ")
            bot = instruction[4:n1]

            if bot in bots:
                if len(bots[bot]) == 2:
                    finished = False
                    lowTo = instruction[n1+14:n2]
                    highTo = instruction[n2+13:]
                    highVal = bots[bot].pop()
                    lowVal = bots[bot].pop()

                    if (lowVal == 17 and highVal == 61): print("bot %s" % bot)

                    if lowTo[:3] == "bot":
                        lowBot = lowTo[4:]
                        if lowBot in bots:
                            bots[lowBot].append(lowVal)
                            bots[lowBot].sort()
                        else:
                            bots[lowBot] = [lowVal]
                    else:
                        lowOutput = lowTo[7:]
                        if lowOutput in outputs:
                            outputs[lowOutput].append(lowVal)
                            outputs[lowOutput].sort()
                        else:
                            outputs[lowOutput] = [lowVal]

                    if highTo[:3] == "bot":
                        highBot = highTo[4:]
                        if highBot in bots:
                            bots[highBot].append(highVal)
                            bots[highBot].sort()
                        else:
                            bots[highBot] = [highVal]
                    else:
                        highOutput = highTo[7:]
                        if highOutput in outputs:
                            outputs[highOutput].append(highVal)
                            outputs[highOutput].sort()
                        else:
                            outputs[highOutput] = [highVal]
